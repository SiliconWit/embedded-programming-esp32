...
:::tip[Instrument it]
BLE is the right link for a sensor that must run for months on a coin cell and only needs to reach a phone or a nearby gateway: a temperature beacon on a cold-store door, a vibration tag on a motor, a wearable. Notify the reading rather than letting the client poll, so the radio sleeps between updates, and a gateway can collect several tags and [forward them onward](/education/iot-systems/) to [siliconwit.io](https://siliconwit.io). It is monitoring without the power cost of Wi-Fi.
:::
