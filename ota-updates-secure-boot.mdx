...
heroImage: "https://cdn.siliconwit.com/education/embedded-programming-esp32/ota-updates-secure-boot.png"
---
...
flowchart TD
    A(["Bootloader starts"]) --> B["Read otadata\nfrom flash"]
    B --> C{"ota_0 or ota_1\nhigher sequence?"}
    C --> D["Select partition\nwith higher seq"]
    D --> E{"State field?"}
    E -->|VALID| F(["Boot selected\npartition"])
    E -->|PENDING_VERIFY| G{"App calls\nmark_valid\nbefore WDT?"}
    G -->|Yes| F
    G -->|No / WDT fires| H["Mark INVALID\non next reboot"]
    E -->|INVALID / ABORTED| I["Try other\nOTA slot"]
    H --> I
    I --> J{"Other slot\nvalid?"}
    J -->|Yes| F
    J -->|No| K(["Boot factory\npartition"])
...
:::tip[Instrument it]
A monitor you deploy is a monitor you will need to change: a new threshold, a bug fix, a better model. OTA is what lets you do that to a node bolted inside a machine or sitting in a field without ever touching it, and secure boot with signed images is what stops anyone else pushing firmware to it. A fleet you can update and trust remotely is the difference between a one-off demo and infrastructure you [run for years](https://siliconwit.io).
:::
