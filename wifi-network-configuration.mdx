...
The ESP32's Wi-Fi stack is a complete 802.11 b/g/n implementation running on dedicated hardware, and ESP-IDF gives you full control over it through an event-driven API. In this lesson you will connect as a station, run as an access point, provision credentials without hardcoding them, and scan the radio spectrum. The project is a portable Wi-Fi signal mapper: walk around with your ESP32 and it builds a table of every access point it can see, complete with signal strength, channel, and encryption type, all served as a self-hosted webpage. #ESP32 #WiFi #Networking
